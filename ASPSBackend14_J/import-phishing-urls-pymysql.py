...
Usage:
------
python import-phishing-urls-pymysql.py phishing_urls.csv
...
def import_csv(csv_file, host='localhost', port=3306, user='root', password=None, database='ASPSBackend2DB'):
    ...
    print("=" * 70)
    print("Phishing URL Import Utility (PyMySQL)")
    print("=" * 70)
    print(f"\nCSV File: {csv_file}")
    ...
    try:
        with open(csv_file, 'r', encoding='utf-8') as f:
            ...
            # Show final count
            cursor.execute("SELECT COUNT(*) as count FROM KnownPhishingWebsites WHERE DateDeleted IS NULL")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import-phishing-urls-pymysql.py <csv_file>")
        print("\nExample CSV format:")
        print("  url,source")
