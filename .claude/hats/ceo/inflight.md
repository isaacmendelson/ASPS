...
- **Post-migration enhancements (deployed + verified live):**
  - **ASPS-747** — tuned the bot system prompt to prefer auto-allowed Read/Grep/Glob over Bash cat/grep, cutting approval friction (user-chosen "smart reduction").
  - **ASPS-748** — added read-only **JIRA + GitHub MCP servers** to the bot (JIRA = `sooperset/mcp-atlassian` Docker pinned by `@sha256` digest, `READ_ONLY_MODE`; GitHub = remote `/mcp/readonly` + Bearer PAT), auto-allowed via `mcp__github__*` / `mcp__mcp-atlassian__*` wildcards. Project questions (JIRA status/assignee, GitHub PRs) now answered with NO Telegram prompt. Verified live. PR #45 — first merge under the new mandatory security gate.
- **Process:** established a **mandatory pre-merge SECURITY gate** (2026-09-07, Isaac's directive) — 3 gates now guard `main`: QA + code review + security. Codified in `.claude/rules/task-workflow.md` + `review-standards.md` + `CLAUDE.md` (surfaced there after the bot omitted it live) + CEO `operating_principles.md`/`decisions.md`.
- **Key learnings:** (1) Claude Agent SDK `settingSources:["project"]` loads `.claude/settings.json` `permissions.allow` which BYPASSES `canUseTool` — use `settingSources:[]` for isolation. (2) Ubuntu 24.04 `ssh.socket` activation + switching to ssh.service without `/run/sshd` = kex-reset lockout; for port 22 keep socket activation. (3) `deploy/vps/` is the reusable provision chain 01→05. (4) Read-only MCP servers can be auto-allowed (bypass `canUseTool`) safely IF read-only by construction (endpoint/flag) + version/digest-pinned; a per-server wildcard is the documented exception to "no `mcp__*` wildcards". (5) **M2 branch protection does NOT independently gate the bot** — the box token is the repo owner's + `enforce_admins=false`, so it bypasses; the real agent-gate is the Telegram approval on every `git push` (Bash). Decision: accept C; follow-up A (separate non-admin machine-user) deferred.
- Docs: handoff `docs/task-memory/VPS_TELEGRAM_MIGRATION_HANDOFF.md`, runbook `docs/cloud/VPS_TELEGRAM_RUNBOOK.md`, hardening `docs/cloud/VPS_TELEGRAM_HARDENING.md`. Optional Minor follow-ups tracked on ASPS-745 (sandbox tightening, token rotation, retire old PAT, least-priv JIRA service account, machine-user for M2). Operator access artifacts at `C:\Jobs\ASPS\` (not in repo).
