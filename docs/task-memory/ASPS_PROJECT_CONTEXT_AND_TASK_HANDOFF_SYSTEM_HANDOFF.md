# ASPS Project Context and Task Handoff System — Task Handoff

**Memory status:** Canonical task handoff  
**Task name:** `ASPS Project Context and Task Handoff System`  
**Task ID:** `019fa4d3-767d-7e71-b8f2-08fead18fa10`  
**Created:** 2026-07-27  
**Renamed:** 2026-07-28  
**Based on:** `docs/task-memory/ASPS_System_Specification_Audit_Handoff.md`  
**Imported source task:** `019f8a0f-1166-7e60-80e9-7a71386ce18d` (`ASPS Specifications create/update`)  
...
- Inspect all system components under `ASPSBackend14_J/`, `apps/`, `Analyzers/`.
- Include the deployment files needed for an end-to-end view.
...
### Cross-Session Continuity Added Later

On 2026-07-27:

- `docs/PROJECT_CONTEXT.md` was created as the mandatory shared ASPS context for all tasks, sessions, and agents.
- Repository-root `AGENTS.md` was updated to require reading `docs/PROJECT_CONTEXT.md` completely at session start, followed by the matching task handoff.
- The handoff convention was defined as `docs/task-memory/<TASK_NAME>_HANDOFF.md`, with one canonical file per task and a matching rename when the Codex task is renamed.
...
1. Create a dedicated specifications document for the Basic URL Analyzer implementation under `Analyzers/basic-url-analyzer/`. The document must be derived from the current Analyzer code and must distinguish implemented behavior, partial behavior, missing behavior, interfaces, configuration, deployment assumptions, tests, and known gaps.
2. Resolve the final naming request: change the specification title, filename, or both to `ASPS System Specification Gaps`. The source task did not clarify which form was intended.
3. Rebuild the Knowledge Engine index after this handoff is accepted.
4. Verify retrieval using queries that target the requirement counts, notification reliability gap, Analyzer specification task, and final naming request.
5. Keep future conclusions synchronized with the canonical specification and current code.
...
- Basic URL Analyzer specifications
- Analyzers basic-url-analyzer missing specification
