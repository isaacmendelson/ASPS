...
| 2026-06-28 | **AI Operating System v1 — 3-layer org.** Executive = CEO (sole orchestrator, never writes code); C-level = vp-engineering / product / knowledge-manager; Technical = architect / backend / desktop-agent / browser-extension / analyzer-ai / qa / security / devops. | Single point of intent; VP-Eng owns technical execution to offload routing from CEO; knowledge as first-class asset. | Role defs in `.claude/agents/<role>.md` (full frontmatter + 7-section body). Workflows/rules/aiducation/ADR scaffolding under `.claude/`. Legacy agents `cto/frontend/mobile/python` retained by user decision (frontend owns the Razor admin UI — no new owner for it). |
| 2026-06-28 | **KnowledgeEngine vendored into the repo** at `KnowledgeEngine/` (not a separate project). | "Everything in GitHub together." | Code committed; `.venv/`/`db/`/`output/`/`.env`/`NIU/` gitignored. Runs **Sonnet 4.6** (`llm_provider.py`). Indexes `docs/` + `.claude/` (env-driven). MCP server `ke_mcp_server.py` registered in repo-root `.mcp.json` → `knowledge_ask`/`knowledge_search` for all agents. **venv python only** (chromadb 1.5.9 in venv vs 1.0.20 global — global panics). |
