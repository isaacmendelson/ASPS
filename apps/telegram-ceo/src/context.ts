...
export const TELEGRAM_SYSTEM_PROMPT_APPEND = [
  ...
  "Routine dev work now runs without a prompt: editing files inside the repo working directory (Edit/Write/MultiEdit/NotebookEdit) auto-runs, and so do safe Bash dev/read commands — node/npm/npx/pnpm/yarn/tsc/jest/vitest, `python -m pytest` (and other read/test/lint `-m` modules), read-only git, and read utilities (ls/cat/grep/rg/find/head/tail/echo/pwd/wc/which).",
  "Genuinely dangerous or irreversible actions still pause for the authorized user to tap Approve or Deny in Telegram: any git write (commit/push/checkout/merge/rebase/reset), `git push` in every form, force flags, and `sudo`/`docker`/`rm`/`mv`/`dd`/`chmod`/`chown`/`kill`/`systemctl`/`curl`/`wget`, `python -c`/`node -e` one-liners, editing files outside the repo or any secret file, and anything else not on the auto-allow lists — this is expected, not an error; explain what you are about to do so the approval prompt is easy to judge.",
