...
- `Write` / `Edit` / `NotebookEdit` / `Bash` / `Task` / `WebFetch` / most MCP
  tools — gated behind a Telegram approval (see below).
...
### 1. Path guard (`src/security.ts` — `checkPathAllowed`)

Every `Read` / `Edit` / `Write` / `NotebookEdit` call (and `Grep`/`Glob`
where a `path` is given) is checked **inside `canUseTool`**, before the tool
ever runs:

- The path is resolved to a real, symlink-free absolute path (`..` and
  symlinked ancestors included).
- It is rejected **unconditionally**, regardless of location, if it matches
  a secret pattern (`SECRET_PATH_PATTERNS` in `security.ts`): `*.env`,
  `*.key`, `*.pem`, `*.pfx`, `id_rsa*`, `*.ppk`, anything named
  `ACCESS_KEYS*`, or any path under a `.ssh`/`.aws`/`.gnupg` segment.
- It is rejected if it falls outside `WORKING_DIR`.
...
- **Everything else** (`Write`, `Edit`, `NotebookEdit`, non-dangerous
  `Bash`, `Task`, `WebFetch`, any other MCP tool, etc.): `canUseTool` calls
  `requestApproval()`, which sends the authorized user an inline-keyboard
  Telegram message (✅ Approve / ❌ Deny) summarizing the tool name and a
  truncated, safe rendering of the input (command or path), and blocks the
  tool call until:
...
- File access is confined to `WORKING_DIR` and secret-pattern paths are always denied, by the path guard described above — not merely by the SDK's default `cwd` scoping.
- **Every state-changing tool call requires an explicit Telegram approval from the authorized user** (see above); the Bash denylist is defense-in-depth on top of that, not a replacement for it.
