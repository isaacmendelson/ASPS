...
- **Process:** mandatory pre-merge **security gate** established (QA + code review + security — all 3 required for `main`); codified in `.claude/rules/task-workflow.md`, `review-standards.md`, `CLAUDE.md`, and CEO hat memory.
- **Finding (ASPS-745):** M2 branch protection does NOT independently gate the bot (box token = repo owner + `enforce_admins=false` → bypasses). Real agent-gate = Telegram approval on every `git push`. Decision: accept C; follow-up A (separate non-admin machine-user account) deferred.
